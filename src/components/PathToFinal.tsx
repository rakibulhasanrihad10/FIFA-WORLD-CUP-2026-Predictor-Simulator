    ...
    <>
              ...
              {filteredTeams.length > 0 ? (
                filteredTeams.map((t) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2.5 flex items-center justify-between hover:bg-slate-900 transition-colors text-xs font-semibold ${
                      t.id === selectedTeamId ? 'bg-slate-900/60 text-[#FFD700]' : 'text-slate-300'
                    }`}
                  >
      ...
      {selectedTeamId && selectedTeam ? (
        <div className="flex flex-col gap-8 animate-fade-in">
          ...
          <div className="flex flex-col gap-2">
            ...
            {isGroupStageActive() ? (
              <div className="flex items-center gap-1.5 text-[10px] text-emerald-400 font-bold self-start bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-lg">
                <Sparkles className="h-3 w-3" />
                Using live group standings to resolve matches
              </div>
            ) : (
              <div className="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold self-start bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-lg">
                <Info className="h-3 w-3" />
                Using FIFA Rankings as default predictor. Click teams on cards to predict custom matchups.
              </div>
            )}
          </div>
                          ...
                          <div className="flex flex-col p-2.5 bg-slate-950/40 border border-slate-800/80 rounded-xl gap-2">
                            ...
                            {bottomTeam ? (
                              <div className="flex items-center gap-2.5 p-2 rounded-xl border border-transparent bg-slate-950/30 text-slate-500 opacity-45 hover:opacity-60 transition-all">
                                ...
                                <span className="text-[9px] font-bold text-slate-500 font-sports-header ml-auto">#{bottomTeam.rank}</span>
                              </div>
                            ) : (
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center border border-dashed border-slate-800 rounded-3xl bg-slate-950/10 select-none animate-fade-in">
          <Sparkles className="h-10 w-10 text-[#FFD700] mb-3 opacity-60 animate-pulse" />
      ...
      {selectedTeam && (
        <div className="absolute left-[-9999px] top-[-9999px] pointer-events-none select-none">
          <div 
            id="export-path-card" 
            className={`bg-[#050b18] text-white p-8 pt-10 pb-10 flex flex-col justify-between items-center relative overflow-hidden ${
              viewMode === 'bracket' ? 'w-[1280px] h-[720px]' : 'w-[800px] h-[1000px]'
            }`}
                  ...
                  return (
                    <div key={stage.matchId} className="flex items-center flex-1">
                      <div className="flex flex-col gap-1.5 w-52 bg-slate-900 border border-slate-700/80 p-3.5 rounded-xl shadow-[0_8px_20px_rgba(0,0,0,0.5)]">
                        <div className={`text-[8.5px] font-black uppercase tracking-widest text-center border-b border-slate-800 pb-1.5 font-sports-header ${stageHeaderColor}`}>
                          {stage.stageLabel}
                        </div>
                        <div className="text-[7.5px] font-black text-slate-500 tracking-wider uppercase text-center mt-0.5">
                          MATCH {stage.matchNumber} • {opponent ? 'COMPLETED' : 'UPCOMING'}
                        </div>
                  ...
                  return (
                    <div key={stage.matchId} className="relative pl-16 flex items-center">
                      ...
                      <div className="w-32 flex-shrink-0 flex flex-col text-left">
                        <span className="text-sm font-black uppercase tracking-wider text-[#FFD700] font-sports-header">
                          {stage.stageLabel}
                        </span>
