...
export default function MatchCard({ match, onSelectWinner, lightTheme = false }: MatchCardProps) {
  // Look up teams
  ...
  const renderTeamRow = (team: Team | undefined, placeholder: string | undefined, isHome: boolean) => {
    ...
    if (!team) {
      return (
        <div className={`flex items-center justify-between p-2.5 rounded-lg border border-dashed text-xs italic ${
          lightTheme
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0">
          <img 
            ...
            alt={`${team.name} Flag`} 
            className={`w-5.5 h-3.5 object-cover rounded-sm shadow flex-shrink-0 select-none ${
        ...
        {isSelected && (
          <span className={`flex items-center justify-center h-4.5 w-4.5 rounded-full flex-shrink-0 ${
  ...
  return (
    <div className={`rounded-xl flex flex-col justify-between gap-2.5 py-3 px-4 h-auto transition-all duration-300 border ${
      lightTheme
        ? isPredictable
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className={`flex items-center justify-center my-1 text-[10px] font-extrabold rounded w-8 mx-auto z-10 select-none border ${
          lightTheme
